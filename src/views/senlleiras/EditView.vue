...
onMounted(async() => {
    try {
        await store.dispatch('species/getListadoEspecies'); //Cargamos el listado de especies
...
const species = computed(() => {
    return store.state.species.speciesFilter;
...
const obtenerNombreComun = e => {
    senlleira.nombreComun = '';
    const specie = species.value.find(element => element.id === e.target.value);
    if (specie) {
        senlleira.nombreComun = specie.names.join();
        senlleira.genus = specie.genus;
        senlleira.specie = specie.specie;
    }
}
